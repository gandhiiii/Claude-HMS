...
export const AppProvider = ({ children }) => {
  ...
  const [trainingRecords, setTrainingRecords] = useState(() => {
    const saved = localStorage.getItem('carepulse_training_records');
    if (saved) {
      try {
        const parsed = JSON.parse(saved);
        if (Array.isArray(parsed)) return parsed;
  ...
  useEffect(() => {
    localStorage.setItem('carepulse_training_records', JSON.stringify(trainingRecords));
  }, [trainingRecords]);

  const addTrainingRecord = (record) => {
    const newRecord = {
      id: `TRN-${String(Date.now()).slice(-6)}`,
      date: new Date().toISOString().split('T')[0],
      ...record
    };
    setTrainingRecords(prev => [newRecord, ...prev]);
    triggerToast(`Training record added for: ${newRecord.participantName}`, 'success');
  };

  const deleteTrainingRecord = (id) => {
    setTrainingRecords(prev => prev.filter(r => r.id !== id));
    triggerToast(`Training record removed (${id})`, 'info');
  };
